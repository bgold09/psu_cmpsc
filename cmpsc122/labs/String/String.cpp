...
String String::Substring(int startPosition, int length/* = 0*/)
{
	...
	if (length == 0)
	{
		String subString( GetLength() + 1 );
		int i = startPosition;
		for ( i; this->Text[i] != '\0'; i++ )
				subString[i] = this->Text[i];
		subString[i] = '\0';
...
/*static*/void String::Copy(char* source, char* destination, int length)
{
	...
	{
		stringstream error;
		error << "String::Copy member function --> destination string '" << destination << "' is not large enough to\
			\ hold " << length << " characters from source string '" << source << "'" << endl;
		throw error.str();
	}
